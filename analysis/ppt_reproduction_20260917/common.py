...
REF=HERE/'reference';OUT=HERE/'results';FIG=HERE/'figures'
sys.path.insert(0,str(REF))
for p in (OUT,FIG):p.mkdir(exist_ok=True)
def tables():
 s=pd.read_parquet(REF/'data/els3_dataset.parquet').sort_values('isu_ord').reset_index(drop=True)
 v=pd.read_parquet(REF/'data/cache/mcvar/mc_variants.parquet')
...
def data(variant,device):
 ...
 curve=s[[f'u{j}' for j in range(10)]].to_numpy() if variant=='A' else v[[f'bu{j}' for j in range(10)]].to_numpy()
