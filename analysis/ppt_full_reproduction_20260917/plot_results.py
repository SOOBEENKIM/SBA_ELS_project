"""Analyze fully recomputed MC labels and retrained OOS predictions; no MC call here."""
from common import *
import matplotlib
matplotlib.use('Agg')
...
def main():
 ...
 ax.axvline(0,color='gray',ls=':');ax.set(xlabel='Fair - MC (KRW, original normalization x 10,000)',ylabel='Number of products',title=f'Fair minus MC: A vs B (n={len(d):,})');ax.legend();save(fig,'fair_mc_histogram')
 ...
 for x in 'AB':
  ax[0].plot(year.index,year['mean_'+x],'-o',label=LAB[x],color=COL[x]);ax[1].plot(year.index,year['MAE_'+x],'-o',color=COL[x])
 ax[0].set(title='Mean fair - MC by issue year',ylabel='Mean gap (KRW)');ax[1].set(title='MAE by issue year',ylabel='MAE (KRW)');ax[0].legend(fontsize=8);save(fig,'fair_mc_by_year')
 dump(OUT/'fair_mc_aggregation.json',dict(population=len(s),included=int(ok.sum()),excluded=int((~ok).sum()),stale=int(v.stale_px.sum()),failed=int(v.fail.notna().sum()),ids_order_match=True,aggregation='Fresh full-population 40000-path MC run; no source MC label reuse',fair_units='Source fair=FAIR_VALUE/ISU_PRC_DETAIL, MC is unit-face. Preserve exact notebook convention; multiplying by 10000 is not common cash normalization for discounted issue prices.'))
 ...
 for ax,m in zip(axes,('deeponet','xgb')):
  limit=max(pd.concat([(preds[(m,x,seed)].mc_pred-preds[(m,x,seed)].mc_true)*10000 for seed in range(5)]).abs().quantile(.995) for x in 'AB')
  for x in 'AB':
   q=pd.concat([preds[(m,x,seed)] for seed in range(5)]);err=(q.mc_pred-q.mc_true)*10000;ax.hist(err,bins=np.linspace(-limit,limit,61),alpha=.5,color=COL[x],label=f'{x}: MAE={abs(err).mean():.1f} KRW')
   for seed in range(5):
 ...
 scale=[]
 for m,x,_ in series:
  p=pd.concat([preds[(m,x,seed)] for seed in range(5)]);e=(p.mc_pred-p.mc_true)*10000;t=preds[(m,x,0)].mc_true*10000;se=v.set_index('item').loc[preds[(m,x,0)].ITEM_CD,'se_'+x]*10000
  scale.append(dict(model=m,variant=x,target_sd=t.std(),prediction_error_sd=e.std(),bias=e.mean(),MAE=e.abs().mean(),MAE_over_target_sd=e.abs().mean()/t.std(),mean_MC_SE=se.mean()))
 pd.DataFrame(scale).to_csv(OUT/'stage1_error_scale.csv',index=False)
 folds=pd.DataFrame(foldrows);fg=folds.groupby(['model','variant','fold'])[['R2','MAE','MAPE']].mean();fig,axes=plt.subplots(1,3,figsize=(13.3,4.6))
 for m,x,col in series:
  q=fg.loc[(m,x)]
  for ax,metric in zip(axes,['R2','MAE','MAPE']):ax.plot(q.index,q[metric],'-o',color=col,label=f'{m} {x}');ax.set(title=metric+' by OOS fold',xlabel='Fold',ylabel=metric);ax.set_xticks(q.index)
 axes[0].legend(fontsize=7);save(fig,'stage1_by_fold')
