...
REF=HERE.parent/'ppt_reproduction_20260917/reference';OUT=HERE/'results';FIG=HERE/'figures';BASE=HERE.parent/'ppt_full_reproduction_20260917/results'
sys.path.insert(0,str(REF))
for p in (OUT,FIG):p.mkdir(exist_ok=True)
def tables(arm='corrected'):
 s=pd.read_parquet(BASE/'rebuilt_universe.parquet')
 v=pd.read_parquet(BASE/'fresh_market_inputs.parquet')
 q=pd.read_parquet(OUT/'paired_prices.parquet').set_index('item')
 ok=v.item.isin(q.index).to_numpy()
 for x in 'AB':
  v['mc_'+x]=v.item.map(q['mc_'+x+'_'+arm]);v['se_'+x]=v.item.map(q['se_'+x+'_'+arm])
 assert s.item.equals(v.item)
 return s,v,ok
def data(variant,device,arm='corrected'):
 from module import data as md
 s,v,ok=tables(arm);s=md.fill_lizard_neutral(s)
...
def training_fingerprint(model,variant,seed,device,arm):
    files=[BASE/'rebuilt_universe.parquet',BASE/'fresh_market_inputs.parquet',OUT/'paired_prices.parquet',REF/'config.yaml',REF/'module/train.py',REF/'module/networks.py',REF/'module/data.py',REF/'model/benchmark.py',REF/'model/deeponet.py',HERE/'common.py',HERE/'train_stage1.py']
    return dict(model=model,variant=variant,seed=seed,device=device,payoff=arm,features='Original 51 contract features held fixed; no schedule/monthly feature augmentation',sha256={str(f.relative_to(HERE.parent)):hashlib.sha256(f.read_bytes()).hexdigest() for f in files})
